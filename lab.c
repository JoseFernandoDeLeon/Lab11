 ...
 * Programa: Comunicación SPI donde el maestro y el esclavos intercambian datos
 * Hardware: potenciómetro en PORTA (maestro), botones en PORTB (esclavo), leds y resistencias en PORTD en ambos
 * 
 * Creado: 9/05/22
...
void __interrupt() isr (void){     
        ...
        if (INTCONbits.RBIF)  
    {
        ...
        if (temp_val != FLAG_SPI){       // Es envío solo para generar los pulsos de reloj?
            PORTD = temp_val;       // Mostramos valor recibido en PORTD
...
void main(void) {
    ...
    while(1){
        ...
        if (PORTAbits.RA0){
            ...
            //PORTB = pot;                    // Mostramos valor del contador del maestro en PORTB
